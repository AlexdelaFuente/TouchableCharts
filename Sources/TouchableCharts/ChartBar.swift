...
public struct ChartBar: View {
    ...
    var barSpacing: CGFloat = 20
        ...
        // Control variable to avoid multiple scrolls to the end
        ...
        public var body: some View {
            VStack {
                ScrollViewReader { scrollViewProxy in
                    ...
                    // Ensures the scroll only happens once when the view appears
                    .onAppear {
                        if !hasScrolledToEnd {
                            scrollViewProxy.scrollTo(viewModel.data.count - 1, anchor: .trailing)
                            animateBarsSequentially()
                            hasScrolledToEnd = true // Mark as scrolled to end
                        }
                    }
                    .padding()
                }
        ...
        private func calculateNumber(from numero: Double) -> Double {
            ...
            return minSalida + (numeroClamped - minNumero) * (maxSalida - minSalida) / (maxNumero - minNumero)
        }
